        ...
        for definition_row_num, definition_param in domain_param.items():
            ...
            for i in range(cycle_time):
                ...
                if seq_field in standard_fields:
                    ...
                    definition_dataset.sort(key=lambda x: [int(x[sortField].replace('TREATMENT', '') if x[sortField] else '0') if sortField == 'EPOCH' else x[sortField] for sortField in sort_keys])
