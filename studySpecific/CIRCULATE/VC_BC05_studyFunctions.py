...
def tableMerge(sort_field, *tableList):
    ...
    # 按照SUBJID和指定字段进行升序排序
    merged_info = merged_info.sort_values(
        by=["SUBJID", sort_field], ascending=[True, True]
    )
...
def make_DMFrame():
    ...
    """
    # 获取格式化数据集
    format_dataset = getFormatDataset(
        "INEX", "SS_A_ALL", "CM_A_CH_PRP", "PAT", "TI_A[VER]"
    )
    ...
    # 初始化主数据框，包含患者基本信息
    dmframe = pat_df[
        ["SUBJID", "INVESTIG", "BRTHDAT", "SEX", "PATINT", "Site_Object_SiteName"]
    ].copy()

    # 处理研究者(INVESTIG)数据，去掉括号和括号内的内容，去掉前后空格
    dmframe["INVESTIG"] = (
        dmframe["INVESTIG"].str.replace(r"（.*?）", "", regex=True).str.strip()
    )

    # 处理入组(INEX)数据
    inex_subset = inex_df[
        ["SUBJID", "RFICDAT", "REGDAT_A", "COHORT", "AGE", "ICINVNAM", "YEARS"]
    ].copy()
    dmframe = pandas.merge(dmframe, inex_subset, how="left", on="SUBJID").fillna("")

    # 处理生存状态(SS_A_ALL)数据
    ss_a_all_df = ss_a_all_df[
        ["SUBJID", "OUTDAT1", "OUTDAT2", "SURVSTAT", "FUDAT", "SSTIMEP"]
    ].astype(str)
    ...
    ss_a_all_df["SURVSTAT_PRIORITY"] = ss_a_all_df["SURVSTAT"].apply(
    ...
    # 合并治疗开始日期到主数据框
    dmframe = pandas.merge(
        dmframe, treatment_start_df[["SUBJID", "CMSTDCH"]], how="left", on="SUBJID"
    ).fillna("")
    ...
    treatment_end_df = treatment_end_df.sort_values(
        by=["SUBJID", "CHSPID"], ascending=[True, False]
    )
    treatment_end_df = treatment_end_df.drop_duplicates(subset=["SUBJID"], keep="first")
    # 合并治疗结束日期到主数据框
    dmframe = pandas.merge(
        dmframe, treatment_end_df[["SUBJID", "CMENDCH"]], how="left", on="SUBJID"
    ).fillna("")

    # 处理版本信息数据
    ti_a_ver_df = ti_a_ver_df[["SUBJID", "CHKVALUE"]].rename(
        columns={"CHKVALUE": "PRTVER"}
    )
...
def process_MH_A_PT():
    ...
    mh_a_pt_df["LYYN"] = mh_a_pt_df["LY_LY1"].where(
        mh_a_pt_df["LY_LY1"].notna() & (mh_a_pt_df["LY_LY1"] != ""), mh_a_pt_df["LYYN"]
    )

    # 处理血管侵犯(VYN)字段，优先使用V_V1
    mh_a_pt_df["VYN"] = mh_a_pt_df["V_V1"].where(
        mh_a_pt_df["V_V1"].notna() & (mh_a_pt_df["V_V1"] != ""), mh_a_pt_df["VYN"]
    )
    ...
    mh_a_pt_df["PNYN"] = mh_a_pt_df["PN_PN1"].where(
        mh_a_pt_df["PN_PN1"].notna() & (mh_a_pt_df["PN_PN1"] != ""), mh_a_pt_df["PNYN"]
    )

    # 处理外科切缘(EXYN)字段，优先使用EX_Y
    mh_a_pt_df["EXYN"] = mh_a_pt_df["EX_Y"].where(
        mh_a_pt_df["EX_Y"].notna() & (mh_a_pt_df["EX_Y"] != ""), mh_a_pt_df["EXYN"]
    )
...
def process_RGACOHD_MH():
    ...
    # 合并数据集
    rgacohd_mh_df = pandas.merge(
        rgacohd_mh_df, rgacohd_df, how="left", on="SUBJID"
    ).fillna("")
...
def process_MH_A():
    ...
    mh_a_df["TNM_T"] = mh_a_df["TNM_T4"].where(
        mh_a_df["TNM_T4"].notna() & (mh_a_df["TNM_T4"] != ""), mh_a_df["TNM_T"]
    )
    ...
    mh_a_df["TNM_N"] = mh_a_df["TNM_N2"].where(
        ...
        mh_a_df["TNM_N1"].where(
            mh_a_df["TNM_N1"].notna() & (mh_a_df["TNM_N1"] != ""), mh_a_df["TNM_N"]
        ),
    )

    # 处理临床M分期(TNM_M)字段，优先使用TNM_M1
    mh_a_df["TNM_M"] = mh_a_df["TNM_M1"].where(
        mh_a_df["TNM_M1"].notna() & (mh_a_df["TNM_M1"] != ""), mh_a_df["TNM_M"]
    )

    # 处理病理T分期(PTNM_T)字段，优先使用PTNM_T4
    mh_a_df["PTNM_T"] = mh_a_df["PTNM_T4"].where(
        mh_a_df["PTNM_T4"].notna() & (mh_a_df["PTNM_T4"] != ""), mh_a_df["PTNM_T"]
    )
    ...
    mh_a_df["PTNM_N"] = mh_a_df["PTNM_N1"].where(
        ...
        mh_a_df["PTNM_N2"].where(
            mh_a_df["PTNM_N2"].notna() & (mh_a_df["PTNM_N2"] != ""), mh_a_df["PTNM_N"]
        ),
    )
    ...
    mh_a_df["TSTAGE"] = mh_a_df["TSTAGET1"].where(
        ...
        mh_a_df["TSTAGET4"].where(
            mh_a_df["TSTAGET4"].notna() & (mh_a_df["TSTAGET4"] != ""), mh_a_df["TSTAGE"]
        ),
    )
    ...
    mh_a_df["NSTAGE"] = mh_a_df["NSTAGEN1"].where(
        ...
        mh_a_df["NSTAGEN2"].where(
            mh_a_df["NSTAGEN2"].notna() & (mh_a_df["NSTAGEN2"] != ""), mh_a_df["NSTAGE"]
        ),
    )
    ...
    mh_a_df["MSTAGE"] = mh_a_df["MSTAGM1C"].where(
        ...
        mh_a_df["MSTAGEM1"].where(
            mh_a_df["MSTAGEM1"].notna() & (mh_a_df["MSTAGEM1"] != ""), mh_a_df["MSTAGE"]
        ),
    )
...
def process_MH_A_RC():
    ...
    supr_mr_full_df = supr_mr_full_df.drop_duplicates(
        subset=["SUBJID", "DAT"], keep="first"
    )

    # 合并数据集
    mh_a_rc_df = pandas.merge(
        mh_a_rc_df, supr_mr_full_df, how="left", on="SUBJID"
    ).fillna("")
...
def process_INEX():
    ...
    merged_df = pandas.merge(inex_df, rgacohb_df, how="outer", on="SUBJID").fillna("")
...
def get_CE():
    ...
    try:
        ...
        return pandas.DataFrame(
            columns=["SUBJID", "CEYN", "CETERM", "CETOXG", "CETERMOT"]
        )
    ...
    result_df = long_df.drop(columns=["_event_num"])[
        ["SUBJID", "CEYN", "CETERM", "CETOXG", "CETERMOT"]
    ]
...
def process_SUPR_SMPL():
    ...
    merged_df["DAT"] = merged_df["SUDAT"].where(
        merged_df["SUTTDATS"] == "TISSUE COLLECTION DATE IS SAME AS THE SURGERY DATE",
        merged_df["SUTTDAT"],
    )

    # 对DAT 字段的数据(一部分是yyyy/mm/dd，一部分是yyyy-mm-dd)进行ISO 8601 格式化(将/变为-，对mm和dd进行补零)
    merged_df["DAT"] = merged_df["DAT"].apply(
...
def process_SUPR_MR():
    ...
    merged_df["DAT"] = merged_df["SUDAT_y"].where(
        merged_df["SUDATS"] == "SAME DAY AS PRIMARY RESECTION", merged_df["SUDAT_x"]
    )
